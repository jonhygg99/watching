...
class GuestStarsSection extends StatefulWidget {
  ...
  const GuestStarsSection({super.key, required this.showId, required this.apiService});
  ...
  void _toggleGuestStars(bool val) {
    setState(() {
      ...
      if (val && _guestStarsCache == null) {
        _guestStarsFuture = widget.apiService.getShowPeople(widget.showId, extended: 'guest_stars').then((data) => data['guest_stars'] as List<dynamic>? ?? []);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Row(
          children: [
            const Text('Mostrar guest stars', style: TextStyle(fontWeight: FontWeight.bold)),
            Switch(
              value: _showGuestStars,
              onChanged: _toggleGuestStars,
            ),
          ],
        ),
        if (_showGuestStars)
          FutureBuilder<List<dynamic>>(
            future: _guestStarsCache != null ? Future.value(_guestStarsCache) : _guestStarsFuture,
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return const Padding(
                  padding: EdgeInsets.symmetric(vertical: 16),
                  child: Center(child: Text('Error cargando guest stars', style: TextStyle(color: Colors.red))),
                );
              }
              final guestStars = snapshot.data ?? [];
              if (_guestStarsCache == null && guestStars.isNotEmpty) _guestStarsCache = guestStars;
              ...
              return Column(
                ...
                children: [
                  ...
                  const Text('Guest stars', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
