...
import 'package:watching/search/trending_grid.dart';
...
class MockShowTranslationService extends Mock implements ShowTranslationService {
  @override
...
void main() {
  ...
  Widget createTestWidget({List<dynamic>? initialTrendingShows}) {
    return ProviderScope(
      overrides: [
        ...
        showTranslationServiceProvider.overrideWithValue(mockTranslationService),
      ],
      child: MaterialApp(
        home: Scaffold(
  ...
  setUp(() {
    ...
    when(mockTranslationService.getTranslatedTitle(
      show: anyNamed('show'),
      traktApi: anyNamed('traktApi'),
    )).thenAnswer((invocation) async {
      final showArg = invocation.namedArguments[const Symbol('show')] as Map<String, dynamic>;
  ...
  testWidgets('uses and translates initialShows when provided', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: MaterialApp(
          home: Scaffold(
            body: TrendingGrid(initialTrendingShows: testShows),
          ),
        ),
      ),
    );
