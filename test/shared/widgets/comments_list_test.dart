...
class TestWrapper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ProviderScope(
      overrides: [
        traktApiProvider.overrideWithValue(traktApi),
      ],
      child: MaterialApp(
        home: Scaffold(
          body: child,
...
void main() {
  ...
  group('showAllComments', () {
    testWidgets('shows modal bottom sheet when button is pressed', (tester) async {
      // Arrange
      when(mockTraktApi.getShowComments(
        id: testShowId,
        sort: 'newest',
        page: 1,
        limit: 10,
    ...
    testWidgets('displays loading indicator when loading comments', (tester) async {
      // Arrange
    ...
    testWidgets('displays error message when loading fails', (tester) async {
      ...
      // Assert
      expect(
        find.text('Error al cargar comentarios: Exception: Failed to load comments'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('loads more comments when scrolled to bottom', (tester) async {
      ...
      final firstPageComments = List<Map<String, dynamic>>.generate(10, (index) => ({
            'id': 'comment-$index',
      ...
      final secondPageComments = List<Map<String, dynamic>>.generate(5, (index) => ({
            'id': 'comment-${index + 10}',
            'comment': 'Comment from second page ${index + 1}',  // Make comment text more distinct
